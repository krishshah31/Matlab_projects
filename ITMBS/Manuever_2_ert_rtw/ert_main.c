...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Manuever_2_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    Manuever_2_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Manuever_2_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Manuever_2_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  UNUSED(argv);
  MW_ev3_init();
  rtmSetErrorStatus(Manuever_2_M, 0);

  /* Initialize model */
  Manuever_2_initialize();
