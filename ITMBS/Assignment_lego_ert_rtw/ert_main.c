...
#include "Assignment_lego_private.h"
#include "rtwtypes.h"
#include "limits.h"
...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    Assignment_lego_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Assignment_lego_M) == (NULL));
  while (runModel) {
    ...
    if (rtmStepTask(Assignment_lego_M, 1)
        ) {
      sem_post(&subrateTaskSem[0]);
    }

    Assignment_lego_step(0);

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Assignment_lego_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Assignment_lego_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  MW_ev3_init();
  rtmSetErrorStatus(Assignment_lego_M, 0);

  /* Initialize model */
  Assignment_lego_initialize();
