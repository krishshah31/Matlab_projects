...
kpi_d = qT_pi_d * (d_alpha(6)*eye(5)+d_alpha(5)*Ad+d_alpha(4)*Ad^2+...
                    d_alpha(3)*Ad^3+d_alpha(2)*Ad^4+Ad^5);
PI_d = -kpi_d(5)*Ts;    %Integral gain
kT_pi_d = kpi_d(1:4)-P*C;    %State feedback value

x1=[1,1,1,1]; %initial Condition
s_obs = [C; C*A; C*A^2; C*A^3]; %controbility matrix
s_obs_inv = inv(s_obs); %inverse of controbility matrix
qT_obs = s_obs_inv(1:1,4); %taking last row of matrix
%setting poles to -3
p1=-3;
p2=-3;
p3=-3;
p4=-3;
alpha_obs=poly([p1, p2, p3, p4]); %conversion to poly

%applying akkermans formula

k_obs = (alpha_obs(5)*eye(4) +alpha_obs(4)*A + alpha_obs(3)*A^2+...
          alpha_obs(2)*A^3 + alpha_obs(1)*A^4)*qT_obs;

h_obs = k_obs; %o/p error feedback gain
h_transp = transpose(h_obs);